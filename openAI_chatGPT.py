# original training loop restored. loss function will need to be updated
...
def generate_tsp_data(num_cities):
    np.random.seed(seed)
    return 100*np.random.rand(num_cities, 3)
...
model=model1(77,num_cities=num_cities)
model.summary()
# Compile the model
...
for epoch in range(num_epochs):
    ...
    model.save(f"./model_{epoch}_loss_{tf.math.reduce_mean(loss)}.keras", overwrite=False)
